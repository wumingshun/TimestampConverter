class TimestampConverter {
  ...
  bindEvents() {
    ...
    if (datetimeInput) {
      ...
      // 处理粘贴事件
      datetimeInput.addEventListener('paste', (e) => {
        e.preventDefault();
  ...
  convertReverse() {
    ...
    // 支持多种格式的时间字符串
    const patterns = [
      /(\d{4})[-/](\d{1,2})[-/](\d{1,2})[ T](\d{1,2}):(\d{1,2}):(\d{1,2})/,  // YYYY-MM-DD HH:MM:SS
      /(\d{4})[-/](\d{1,2})[-/](\d{1,2})[ T](\d{1,2}):(\d{1,2})/,        // YYYY-MM-DD HH:MM
    ...
    const east8Date = new Date(Number(date.year), Number(date.month) - 1, Number(date.day), Number(time.hour), Number(time.minute), Number(time.second));
